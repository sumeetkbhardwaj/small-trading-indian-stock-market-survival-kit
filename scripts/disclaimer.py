"""SEBI Ad-Code-safe verbatim disclaimer surfaced on every user-facing output.
No assured/guaranteed/target-return/superlative language (spec M12/CR-42)."""
DISCLAIMER = (
    "Investment in securities market are subject to market risks. "
    "Read all the related documents carefully before investing. "
    "This is decision-support for research and educational use only, not investment "
    "advice; the author is not a SEBI-registered Investment Adviser or Research Analyst. "
    "You are solely responsible for your own decisions."
)
